...
When looking for sensitivity to external stimuli is often good to stick to some well known metrics.One of them is sleep efficiency, defined as:
...
Given the pre-sleep questionaries of the Human Sleep Project and the sleep stages of each subject I've elaborated a user profile of around 4.6m sessions of different subjects. Computing the sleep efficiency of each of those subjects we obtain:

\begin{figure}[H]
\centering
\hspace*{-2cm}\includegraphics[scale=.5]{figs/plot-se.png}
\caption{Sleep Efficiency in the Human Sleep Project}
\label{fig9}
\end{figure}

As one can see the median sleep efficiency for the sample is around 85\% what is a correct metric, however a lot of outliers below the lower quartile appear in the image too, this is more likely due to the fact that the dataset is composed of people with some kind of sleep pathology. 

Another important aspect in sleep quality is how much sleep time out of bed a subject have. There is a metric called Epworth Sleepiness Scale (ESS), which mesures the tendency to fall asleep during common daily activities, the subject needs to answer to the following eight questions ranging from 0 (very low chance) to 3 (very high chance) of falling asleep:

\begin{itemize}
  \item How likely to fall asleep while sitting reading?
  \item How likely to fall asleep while watching TV?
  \item How likely to fall asleep while sitting in public?
  \item How likely to fall asleep while going by car as passanger during one hour?
  \item How likely to fall asleep while laying down in the afternoon?
  \item How likely to fall asleep while sitting and talking?
  \item How likely to fall asleep while sitting quiet after lunch?
  \item How likely to fall asleep while stopped few minutes in traffic?    
\end{itemize}

The sum of the responses to this questions will yield the ESS punctuation, being from 0 to 10 a normal puntuation while above 10 may suggest excessive daytime sleepiness, which could be a sign of an underlying condition or disorder.

In the dataset taken the ESS looks as follows:

\begin{figure}[H]
\centering
\includegraphics[scale=.5]{figs/plot-ess.png}
\caption{ESS in the Human Sleep Project}
\label{fig10}
\end{figure} 

A median around 7 and the upper quartile above 10, again this may be due to the pathologies suffered by the subjects of the human sleep project.
