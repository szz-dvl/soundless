...
The soundless dataset is composed of 362 sessions for 44 users. In the different sessions the decibels in the ambient are recorded, and the different sleep stages of each patient are recorded too. With this data, we can try to compare the users in HSP project and in Soundless dataset to see if noise is affecting the sleep quality of the patiens.
...
The plot is very similar to the first one, what is natural since the vast majority of the samples come from the HSP dataset, lets see what happens when we plot the sleep efficiency for soundless users:
...
As we can see the sleep efficiency drops dramatically for Soundless users, and the outliers present are above the upper quartile instead of below the lower quartile. 

Goig a little bit further we can try to plot the mean of the decibels for each soundless session using a boundary for the sleep efficiency metric, so, soundless users with a sleep efficiency of 0.6 or above are affected by decibels in the following way:
...
As we can see the decibels median is slightly above 32dB, while the upper quartile is slightly below 60, which is a pretty loud noise, however this patiens are able to sleep the 60\% of the time in bed, now looking at the other side, it is, plotting the decibels for soundless users with an sleep efficiency of less than 0.6 we see:
