...
Given the sleep stages of each subject I've elaborated a user profile of around 12.1m sessions of different subjects of both HSP and soundless dataset. Computing the sleep efficiency of each of those subjects we obtain:

\begin{figure}[H]
\centering
\hspace*{-2cm}\includegraphics[scale=.5]{figs/plot-se-all.png}
\caption{Sleep Efficiency in the Human Sleep Project}
\label{fig9}
\end{figure}

As one can see the median sleep efficiency for the sample is around 85\% what is a correct metric, however a lot of outliers below the lower quartile appear in the image too, this is more likely due to the fact that the dataset is composed of people with some kind of sleep pathology and soundless users, exposed to noise during sleep are included in the plot.\newpage

Now if we compute the same plot for HSP users we see:

\begin{figure}[H]
\centering
\hspace*{-2cm}\includegraphics[scale=.5]{figs/plot-se-hsp.png}
\caption{Sleep Efficiency in the Human Sleep Project}
\label{fig10}
\end{figure}

The plot is very similar to the first one, what is natural since the vast majority of the samples come from the HSP dataset,we have a median of 85\% and a lot of outliers under the lower quartile again. This indicates that the vast majority of the people sleeping in a very controlled environment have a good sleep efficiency, and the outliers are probably due to the fact that the analised people have an sleep pathology, whether it is pain related or related to neurological causes. \newpage

Lets see what happens when we plot the sleep efficiency for soundless users:

\begin{figure}[H]
\centering
\hspace*{-2cm}\includegraphics[scale=.5]{figs/plot-se-soundless.png}
\caption{Sleep Efficiency in the Soundless dataset}
\label{fig11}
\end{figure}

As we can see the sleep efficiency drops dramatically for Soundless users, and the outliers present are above the upper quartile instead of below the lower quartile. This figure shows that users exposed to high dB during the night have a worst sleep efficiency than users in a very controlled environment, even when the users have some sleep pathology.\newpage

Going a little bit further we can try to plot the mean of the decibels for each soundless session using a boundary for the sleep efficiency metric, so, soundless users with a sleep efficiency of 0.6 or above are affected by decibels in the following way:

\begin{figure}[H]
\centering
\hspace*{-2cm}\includegraphics[scale=.5]{figs/plot-db-upper.png}
\caption{Decibels in Soundless dataset for sleep efficiency over 0.6}
\label{fig12}
\end{figure}
 
As we can see the decibels median is slightly above 32dB, while the upper quartile is slightly below 60, which is a pretty loud noise, however this patiens are able to sleep the 60\% of the time in bed.
\newpage
Now looking at the other side, it is, plotting the decibels for soundless users with an sleep efficiency of less than 0.6 we see:

\begin{figure}[H]
\centering
\hspace*{-2cm}\includegraphics[scale=.5]{figs/plot-db-lower.png}
\caption{Decibels in Soundless dataset for sleep efficiency under 0.6}
\label{fig13}
\end{figure}

In this case the median is in the same range of the first case, however, we can see that although the upper quartile is below 50dB we have a lot of outliers above this upper quartile, meaning that people that sleep less is affected by higher decibels.
